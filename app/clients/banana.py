...
class BananaClient:
    ...
    async def generate_pixel_art(self, prompt: str) -> str:
        """
        Generates an image using Google's Gemini-2.5-flash-image model.
        Returns the local URL of the generated image.
        """
        full_prompt = f"pixel art, gacha sprite, {prompt}, white background, high quality"
        ...
        def _generate():
            response = self.client.models.generate_content(
                model="gemini-2.5-flash-image", # Using 2.0-flash or 2.5 if available as per user request example implies newer models
        ...
        if not image_url:
             ...
             raise Exception("No image data found in response. Ensure the model supports image generation.")
